  ...
  return (
    <div className="p-pad-2 max-w-lg">
      <div className="mb-pad-2">
        <Link
          href="/learn"
          className="text-my-stone hover:text-my-cream text-xs uppercase tracking-widest transition-colors"
        >
          ← Learn
        </Link>
      </div>
      ...
      <div className="mb-8">
        ...
        <h1 className="text-my-cream text-2xl font-light mt-2 leading-snug">
      ...
      <ul className="flex flex-col gap-0">
        {series.lessons.map((lesson, idx) => (
          <li key={lesson.slug} className="border-b border-my-espresso/30 last:border-0">
            <Link
              href={`/learn/${series.slug}/${lesson.slug}`}
              className="group flex items-baseline gap-4 py-4"
            >
              <span className="text-my-stone/40 text-xs w-5 shrink-0">{idx + 1}</span>
              <div>
                <span className="text-my-cream text-sm group-hover:text-my-orange transition-colors">
                  {lesson.metadata.title}
                </span>
                {lesson.metadata.description && (
                  <p className="text-my-stone text-xs mt-1 leading-relaxed">
                    {lesson.metadata.description}
                  </p>
                )}
              </div>
      ...
      {series.lessons.length > 0 && (
        <div className="mt-8">
          <Link
            ...
            className="text-my-orange text-xs uppercase tracking-widest hover:text-my-cream transition-colors"
          >
            Start lesson 1 →
          </Link>
        </div>
